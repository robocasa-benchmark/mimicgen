...
class MG_TurnOnToasterOven(RobosuiteInterface):
    ...
    def get_object_poses(self):
        ...
        return dict(
            knob=self.get_object_pose(obj_name=self.env.toaster_oven.name + "_knob_time_main", obj_type="geom"),
        )
    ...
    def get_subtask_term_signals(self):
        ...
        knob_body = find_elements(
            self.env.toaster_oven.worldbody,
            tags="body",
            attribs={"name": "{}_knob_time".format(self.env.toaster_oven.name)},
            return_first=True,
        )
...
class MG_AdjustToasterOvenTemperature(RobosuiteInterface):
    ...
    def get_object_poses(self):
        ...
        return dict(
            knob=self.get_object_pose(obj_name=self.env.toaster_oven.name + "_knob_temp_main", obj_type="geom"),
        )
    ...
    def get_subtask_term_signals(self):
        ...
        knob_body = find_elements(
            self.env.toaster_oven.worldbody,
            tags="body",
            attribs={"name": "{}_knob_temp".format(self.env.toaster_oven.name)},
            return_first=True,
        )
        knob_geoms = find_elements(knob_body, tags="geom", return_first=False)
        knob_geom_names = [e.get("name") for e in knob_geoms]
