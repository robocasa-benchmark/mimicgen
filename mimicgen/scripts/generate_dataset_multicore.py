...
def update_miss(env, num_attempts):
    env.update_env("resample_objects", False) #tells environment if it should generate new config
    num_attempts.value += 1
    print(num_attempts.value)
    
def make_new_dataset_multiprocess(process_id, mg_config, misc_data, total_success, total_failures, total_attempts, total_problematic, ep_lengths, lock):
    
    new_dataset_folder_path = misc_data["new_dataset_folder_path"]
    source_dataset_path = misc_data["source_dataset_path"]
    
    write_video = False
    if process_id == 0:
        write_video = misc_data["write_video"]
    
    render = misc_data["render"]
    video_path = misc_data["video_path"]
    video_skip = misc_data["video_skip"]
    pause_subtask = misc_data["pause_subtask"]
    tmp_dataset_folder_path = misc_data["tmp_dataset_folder_path"]
    tmp_dataset_failed_folder_path = misc_data["tmp_dataset_failed_folder_path"]
    start_time = misc_data["start_time"]
    json_log_path = misc_data["json_log_path"]
    render_image_names = misc_data["render_image_names"]
    
    # log terminal output to text file
    RobomimicUtils.make_print_logger(txt_file=os.path.join(new_dataset_folder_path, 'log' + str(process_id) + '.txt'))
    
    # get environment metadata from dataset
    env_meta = get_env_metadata_from_dataset(dataset_path=source_dataset_path)
    
    # update the env meta with specified target env kwargs (if any)
    env_meta["env_kwargs"].update(mg_config.experiment.task.get("kwargs", {}))
    ...
    env = RobomimicUtils.create_env(
        ...
        seed=mg_config.experiment.seed * 100 + process_id,
    )
    ...
    print("Process {} starting data generation...".format(process_id))
    ...
    while True:
        ...
        num_success = total_success.value
        num_failures = total_failures.value
        num_attempts = total_attempts.value
        num_problematic = total_problematic.value
        ...
        if success or (num_attempts % 50) == 0:
            print("")
            print("*" * 50)
            print("Process {} trial {} success: {}".format(process_id, num_attempts, success))
    ...
    print("")
    print("Process {} finished data generation...".format(process_id))    
...
def generate_dataset_multiprocess(
    ...
    ### Generate data on multiple processes
    
    num_processes = args.num_procs #number of processes to run
    processes = []
    total_success = multiprocessing.Value('i', 0)
    total_failures = multiprocessing.Value('i', 0)
    total_attempts = multiprocessing.Value('i', 0)
    total_problematic = multiprocessing.Value('i', 0)
    lock = multiprocessing.Lock()
    
    manager = multiprocessing.Manager()
    ep_lengths = manager.list() # episode lengths for successfully generated data
    ...
    misc_data = {
        ...
        "video_skip": video_skip,
        "pause_subtask": pause_subtask,
        "tmp_dataset_folder_path": tmp_dataset_folder_path,
        "tmp_dataset_failed_folder_path": tmp_dataset_failed_folder_path,
        "start_time": start_time,
    ...
    for process_id in range(num_processes):
        process = multiprocessing.Process(target=make_new_dataset_multiprocess, args=(process_id, mg_config, misc_data, total_success, total_failures, total_attempts, total_problematic, ep_lengths, lock))
    ...
    ### Finished generating data on multiple processes
...
def main(args):
    ...
    with mg_config.values_unlocked():
        ...
        if args.debug:
            ...
            mg_config.experiment.generation.num_trials = 30
    ...
    try:
        important_stats = generate_dataset_multiprocess(
            mg_config=mg_config,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="(optional) number of parallel processes to run",
    )
